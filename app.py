...
if 'portrait_cache' not in st.session_state:
    st.session_state.portrait_cache = {} # {persona: {'status': str, 'image': img, 'model': str, 'error': str}}

def trigger_portrait_generation(persona):
    """Starts background generation if not already cached or generating."""
    if persona not in st.session_state.portrait_cache:
        st.session_state.portrait_cache[persona] = {
            'status': 'generating',
            'image': None,
            'model': None,
            'error': None
        ...
        def generate_task(p):
            try:
                img, model = generate_journalist_portrait(p)
                if img:
                    st.session_state.portrait_cache[p]['image'] = img
                    st.session_state.portrait_cache[p]['model'] = model
                    st.session_state.portrait_cache[p]['status'] = 'completed'
                else:
                    st.session_state.portrait_cache[p]['status'] = 'failed'
                    st.session_state.portrait_cache[p]['error'] = "No image returned"
            except Exception as e:
                print(f"Background image generation failed: {e}")
                st.session_state.portrait_cache[p]['status'] = 'failed'
                st.session_state.portrait_cache[p]['error'] = str(e)

        thread = threading.Thread(target=generate_task, args=(persona,))
        thread.start()
...
with st.sidebar:
    ...
    # Trigger generation immediately when persona is selected/loaded
    trigger_portrait_generation(persona)
...
if st.session_state.session.is_active:
    ...
    with col1:
        # Image Display Logic (Read from Cache)
        cache_data = st.session_state.portrait_cache.get(st.session_state.session.persona, {})
        status = cache_data.get('status', 'idle')
        
        if status == "completed" and cache_data.get('image'):
            st.image(cache_data['image'], caption=f"Generated by {cache_data.get('model')}", width="stretch")
        elif status == "generating":
            st.info("Generating portrait...")
        elif status == "failed":
            st.warning("Portrait generation failed.")
            if cache_data.get('error'):
                st.caption(f"Error: {cache_data['error']}")
            
        st.info(f"**Interviewer:** {st.session_state.session.journalist_name}")
        st.success(f"**Outlet:** {st.session_state.session.outlet_name}")
