...
def generate_image_background(persona, session):
    """Background task to generate journalist image."""
    try:
        img, model = generate_journalist_portrait(persona)
        session.journalist_image = img
        session.journalist_image_model = model
...
def start_interview():
    ...
    with st.spinner("Generating journalist persona..."):
        ...
        thread = threading.Thread(target=generate_image_background, args=(persona, st.session_state.session))
        thread.start()
...
if st.session_state.session.is_active:
    ...
    with col1:
        if st.session_state.session.journalist_image:
            st.image(st.session_state.session.journalist_image, caption=f"Generated by {st.session_state.session.journalist_image_model}", width="stretch")
        else:
            st.info("Generating interviewer portrait...")
            
        st.info(f"**Interviewer:** {st.session_state.session.journalist_name}")
        st.success(f"**Outlet:** {st.session_state.session.outlet_name}")
        st.markdown(f"*{st.session_state.session.persona}*")
